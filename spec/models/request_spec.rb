...
RSpec.describe Request, type: :model do
  ...
  [:bag_id, :user_id, :external_id].each do |field|
    it "#{field} is required" do
      expect(Fabricate.build(:request, field => nil)).to_not be_valid
    end
  end
  ...
  %w(bag_id external_id).each do |field|
    describe "#{field}" do
      it "must be unique" do
        expect {
          2.times { Fabricate(:request, field.to_sym => "test") }
  ...
  it "has an upload path based on the user and the bag id" do
    ...
    request = Fabricate.build(:request, user: user, bag_id: 1)
  ...
  describe "#to_param" do
    it "uses the bag id" do
      bag_id = 'made_up'
      expect(Fabricate.build(:request, bag_id: bag_id).to_param).to eq(bag_id)
    end
  end

  describe "#external_validation_cmd" do
    it "returns a path to a command" do
      expect(Fabricate.build(:request).external_validation_cmd).not_to be_nil
    end
  end
